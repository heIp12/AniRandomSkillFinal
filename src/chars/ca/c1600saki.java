...
public class c1600saki extends c00main{
	...
	boolean isKang(mj p, int loc) {
		...
		if(loc7 != -1) {
			...
			if(dora.size() < 4) dora.add(AMath.random(set.length)-1);
			ARSystem.playSound((Entity)player, "c1016pk");
	...
	public boolean skill4() {
		if(player.isSneaking()) {
			...
			ARSystem.playSound((Entity)player, "c1016get");
			ARSystem.playSound((Entity)player, "c1016nt");
			cooldown[4] = (float) (20 * (skillmult+sskillmult));
			handTitle();
		} else {
			String key = "";
			String ptype = "";
			...
			List<String> pb = score(hand[loc], loc);
			...
			if(targets.size() <= 0) player.sendTitle("대상없음", "");
			if(pb.size() > 0 && targets.size() > 0) {
				cooldown[4] = 100;
				...
				int time = 1;
				if(targets.size() <= 1) {
					Title("론", "§c[§f"+key+"§c]§f",10,40,0);
					sound("c1016prun");
				} else {
					Title("츠모", "§c[§f"+key+"§c]§f",10,40,0);
					sound("c1016ptumo");
				}
				for(String s : pb) {
					if(s.contains(":")) {
						...
						String text  = ""+p;
						if(p >= 200) text = "§5§lDY§a";
						else if(p >= 100) text = "§4§lY§a";
						String msg = text;
						score += p;
						int damage = score;
						delay(()->{
				...
				if(score > 0) {
					...
					delay(()->{
						...
						for(Player p : Bukkit.getOnlinePlayers()) p.spigot().sendMessage(Text.hover(player,"§e§l※§a§n"+player.getName()+ "§e§l]>§f §f"+k+"§f" + ty, msgs));
						player.sendTitle(sc+" 판 §f§l|§f "+dg+" 점","" + " ",10,80,0);
						
						if(dg < 8000) {
							Title(dg+" 점","" + " ",20,80,0);
							delay(()->{Title(sc+" 판 §f§l|§f "+dg+" 점",ty,0,80,0);},5);
						} else {
							delay(()->{Title(sc+" 판 §f§l|§f "+dg+" 점",ty,0,80,0);},5);
						}
						sound("c1016c"+ttp);
	...
	List<String> score(mj p, int loc) {
		...
		if(rot >= 4 && kan >= 2 && !stop && dr >= 2) {
			Rule.playerinfo.get(player).tropy(16, 7);
			ym.add("t30");
		}
		if(tnd) ym.add("t27");
		if(!stop && dack.size() == 54) {
