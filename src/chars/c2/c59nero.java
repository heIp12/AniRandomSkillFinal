...
public class c59nero extends c00main{
	...
	public void skill0(){
		...
		delay(new Runnable() {
			...
			public void run() {
				skill("c59_spe2");
				player.performCommand("tm anitext all SUBTITLE true 60 c59:t2/"+player.getName());
			}
		},44);
		delay(new Runnable() {
			@Override
	...
	public boolean tick() {
		if(tk%20 == 0 && psopen) {
			scoreBoardText.add("&c ["+Main.GetText("c59:sk0")+ "] : "+ AMath.round(sp*0.05,0) +" / 20");
		}
		if(!isps) {
			if(AMath.random(400) == 2) {
				player.setNoDamageTicks(1);
				player.damage(1);
			}
			if(AMath.random(500) == 5) {
