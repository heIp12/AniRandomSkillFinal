...
public class UpItem037 extends upitemBase{
	public UpItem037(Player p){
		super(p);
		itemCode = 37;
		setcooldown = 40;
	}
	...
	public boolean onAttack(EntityDamageByEntityEvent e) {
		if(Rule.c.get(e.getEntity()) != null && Rule.c.get(player).number == 39) {
			...
			ARSystem.addBuff(en, new Stun(en), 6);
			ARSystem.addBuff(en, new Silence(en), 6);
	...
	public boolean skillCast(){
		...
		for(Entity et : ARSystem.box(player, new Vector(15,12,15), box.TARGET)) {
			delay(()->{
				LivingEntity en = (LivingEntity)et;
				ARSystem.giveBuff(en, new Timeshock(en), 140);
			},10);
		}
		return false;
	}
