...
public class Item020 extends itemBase{
	public Item020(Player p){
		super(p);
		itemCode = 20;
	}
	...
	public void onDeath(Player p, Entity e) {
		if(e != player && Rule.c.get(player) != null) {
			ARSystem.giveBuff(player, new PowerUp(player), 60,0.5);
			Rule.c.get(player).skillmult += 1;
			delay(()->{
				if(Rule.c.get(player) != null )Rule.c.get(player).skillmult -= 1;
			},60);
		}
	}
	...
	protected void onStart() {
		if(Rule.c.get(player).number == 100) {
			...
			Rule.c.get(player).setcooldown[3] *= 0.5;
	...
	public boolean onHit(EntityDamageByEntityEvent e) {
		if(Rule.c.get(player).number == 100 && AMath.random(10) <= 4) {
