...
public class Item012 extends itemBase{
	public Item012(Player p){
		super(p);
		itemCode = 12;
	}
	@Override
	public void onDeath(Player p, Entity e) {
		if(e == player) {
			...
			delay(()->{
				ARSystem.potion(player, 14, 60, 4);
			},2);
		}
	}
	...
	public boolean onHit(EntityDamageByEntityEvent e) {
		if(Rule.c.get(player).number == 13 && AMath.random(10) <= 7) {
			for(int i =0; i<10; i++) if(Rule.c.get(player).cooldown[i] > 0) Rule.c.get(player).cooldown[i] -= 0.5;
