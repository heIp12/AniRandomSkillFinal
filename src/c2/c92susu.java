...
public class c92susu extends c00main{
	...
	public c92susu(Player p,Plugin pl,c00main ch) {
		...
		if(ARSystem.gameMode == modes.LOBOTOMY) maxtick = 1800;
	}
			...
			if(tk%10 == 0) {
				for(Entity e : ARSystem.box(player, new Vector(14,14,14), box.ALL)) {
					if(e.getLocation().distance(player.getLocation()) < 3) {
						...
						skill("c92_move");
						if(!(owner instanceof Player)) owner.setMaxHealth(owner.getMaxHealth() + 100);
						owner.setMaxHealth(owner.getMaxHealth()*1.4);
						owner.setHealth(owner.getMaxHealth());
						if(Rule.c.get(owner) != null) {
							for(int i=0;i<10;i++) Rule.c.get(owner).setcooldown[i] *= 0.8;
						}
					} else {
						if(psound <= 0) {
							psound = 100;
