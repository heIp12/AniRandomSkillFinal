...
public class c60gil extends c00main{
	...
	public void PlayerSpCast(Player p) {
		if(Rule.c.get(p) != null && !(Rule.c.get(p) instanceof c60gil)) {
			...
			if(ARSystem.AniRandomSkill.startplayer.size() > 10 && AMath.random(10) <= 5) {
				count++;
			}
			if(count >= ARSystem.AniRandomSkill.startplayer.size()/2&& skillCooldown(0)) {
				...
				if(Rule.c.get(p).getCode() == 30 && ARSystem.isTarget(p, player,box.TARGET)) {
					...
					if(player.getLocation().distance(p.getLocation()) < 20) {
						...
						player.teleport(Local.lookAt(player.getLocation(), p.getLocation()));
						p.teleport(Local.lookAt(p.getLocation(), player.getLocation()));
						delay(()->{
							...
							for(int i=0;i<60;i++) {
								delay(()->{
									...
									loc = Local.offset(loc, new Vector(-2,1+AMath.random(100)*0.05,4-AMath.random(160)*0.05));
									loc = Local.lookAt(loc, p.getLocation());
	...
	public boolean tick() {
		...
		if(tk%10 == 0) {
			if(sword < 200) {
				...
				if(ARSystem.gameMode == modes.LOBOTOMY) sword += 2;
			} else {
		...
		if(sk2 && sword > 0 && tk%2 == 0) {
			sword--;
			Location l = Local.offset(gob.clone(), new Vector(-2,1+AMath.random(100)*0.05,4-AMath.random(160)*0.05));
