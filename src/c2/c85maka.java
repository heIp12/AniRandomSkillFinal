			...
			delay(()->{
				...
				if(e.size() > 0) {
					...
					for(LivingEntity en : e) {
						en.teleport(Local.offset(player.getLocation(), new Vector(-5,0,0)));
					...
					player.teleport(Local.lookAt(player.getLocation(), Local.offset(player.getLocation(), new Vector(-5,0,0))));
					skill("c85_sp");
		...
		if(isAttack) {
			...
			if(p >= 100) {
				...
				if(!pp) {
					...
					if(ARSystem.gameMode == modes.LOBOTOMY) Rule.buffmanager.selectBuffValue(player, "barrier", (float) player.getMaxHealth() * 3);
				}
			}
