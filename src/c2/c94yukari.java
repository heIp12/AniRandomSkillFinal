...
public class c94yukari extends c00main{
	...
	public boolean skill1() {
		if(p > 20) {
			...
			own_loc = Local.offset(player.getLocation(),new Vector(4,0,0));
	...
	public boolean skill2() {
		if(p > 30) {
			...
			for(int i=0; i< AMath.random(20)+5; i++) skill("c94_s2");
		} else {
			cooldown[2] = 0;
		}
		return true;
	...
	public boolean skill4() {
		if(own_loc != null && tow_loc != null && p >= 100 && skillCooldown(0)) {
			...
			for(int i =0; i<10; i++) {
				delay(()->{
					ARSystem.spellLocCast(player, Local.lookAt(own_loc, tow_loc), "c94_sp");
	...
	public boolean entitydamage(EntityDamageByEntityEvent e, boolean isAttack) {
		if(isAttack) {
			...
			if(ARSystem.gameMode == modes.LOBOTOMY) e.setDamage(e.getDamage()*0.2);
		}
		return true;
	}
}
