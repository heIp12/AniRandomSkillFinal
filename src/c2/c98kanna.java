...
public class c98kanna extends c00main{
	...
	public c98kanna(Player p,Plugin pl,c00main ch) {
		...
		text();
		c = this;
		if(ARSystem.gameMode == modes.KANNA) setcooldown[3] = 10000;
	...
	public boolean skill2() {
		if(target != null) {
			...
			if(target.getHealth()-damage < 1) {
				...
				if(ARSystem.gameMode != modes.KANNA)  ARSystem.heal(player, 5);
	...
	public boolean skill3() {
		...
		if(ARSystem.gameMode == modes.LOBOTOMY) light *= 0.2;
		return true;
	}
	
	public void te() {
		if(target.hasPotionEffect(PotionEffectType.INVISIBILITY) && (target instanceof Player || ARSystem.gameMode == modes.KANNA)) target.removePotionEffect(PotionEffectType.INVISIBILITY);
	...
	public void makerSkill(LivingEntity target, String n) {
		if(n.equals("1")) {
			float kill = 14;
			if(ARSystem.gameMode == modes.LOBOTOMY) kill = 30;
			...
			if(this.target == null) {
				...
				if(ARSystem.gameMode != modes.KANNA) ARSystem.giveBuff(target, new Noattack(target), 200);
				cooldown[1] = 300;
			}
		}
		if(n.equals("2")) {
	...
	public boolean tick() {
		...
		if(target != null && sk1 > 0) {
			...
			if(tk%20 == 0) {
				float damage = 0.5f;
				if(ARSystem.gameMode == modes.LOBOTOMY) damage = 3;
				if(target.getHealth()-damage < 1) {
					count++;
	...
	public boolean entitydamage(EntityDamageByEntityEvent e, boolean isAttack) {
		if(isAttack) {
			...
			if(Rule.c.get(e.getDamager()) != null && Rule.c.get(e.getDamager()) instanceof c63micoto) {
				light += e.getDamage()*2;
