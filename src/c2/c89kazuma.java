...
public class c89kazuma extends c00main{
		...
		if(target != null && Rule.c.get(target) != null) {
			if(AMath.random(100) <= 5 && skillCooldown(0)) {
				...
				if(type == 2) {
					...
					player.teleport(Local.lookAt(player.getLocation(), target.getLocation()));
	...
	public boolean entitydamage(EntityDamageByEntityEvent e, boolean isAttack) {
		if(stil != null) {
			try {
				return stil.entitydamage(e, isAttack);
			} catch(Exception ex) {
