...
public class c8603iriya extends c00main{
	...
	public void pattan1(LivingEntity target) {
		if(target != null) {
			Location loc = Local.lookAt(player.getLocation(), target.getLocation().clone().add(new Vector(1-0.2*AMath.random(10),1-0.2*AMath.random(10),1-0.2*AMath.random(10))));
			if(AMath.random(3) == 2) loc = Local.lookAt(player.getLocation(), target.getLocation().add(target.getVelocity().multiply(1.5)));
	...
	public void pattan2(LivingEntity target) {
		if(target != null) {
			if(delay <= 0) {
				Location loc = Local.lookAt(player.getLocation(), target.getLocation().clone());
			...
			if(delay <= 0) {
				...
				if(range < 15) {
					...
					if(player.isOnGround()) {
						...
						delay(()->{
							Location loc2 = Local.lookAt(player.getLocation(), target.getLocation().clone());
	...
	public void pattan4(LivingEntity target) {
		if(delay <= 0) {
			if(target != null && AMath.random(3) == 2) {
				...
				Location loc = Local.lookAt(ll, target.getLocation().clone());
				player.teleport(loc);
		...
		if(isAttack) {
			...
			if(isps) {
				if(e.getDamager().getLocation().distance(player.getLocation()) > 8 && AMath.random(10) <= 7) {
					...
					ARSystem.spellLocCast(player, Local.lookAt(player.getLocation().clone().add(0,1,0), e.getDamager().getLocation()), "c3086p");
				}
				if(isps) {
