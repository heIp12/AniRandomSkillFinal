...
public class c100kuroko extends c00main{
	...
	public boolean skill4() {		
		...
		if(target != null) {
			...
			loc = Local.lookAt(target.getLocation(),ploc);
			loc = loc.add(loc.getDirection().multiply(1.5));
			loc = Local.lookAt(loc, target.getLocation());
			player.teleport(loc);
						...
						if(Rule.c.get(e) != null && Rule.c.get(e).s_kill >=2 && skillCooldown(0)) {
							...
							for(int i=0;i<10;i++) {
								delay(()->{
									...
									eloc = Local.offset(eloc, new Vector(2,0,0));
									eloc = Local.lookAt(eloc, e.getLocation());
							...
							for(int i=0;i<3;i++) {
								delay(()->{
									...
									eloc = Local.lookAt(eloc, e.getLocation());
									e.teleport(eloc);
									player.teleport(Local.offset(eloc,new Vector(-1,0,0)));
									e.setNoDamageTicks(0);
									e.damage(3,player);
								},60 + i*5);
							}
							for(int i=0;i<2;i++) {
								delay(()->{
									...
									eloc.setYaw(AMath.random(360));
									eloc.setPitch(-10+AMath.random(20));
									eloc = Local.offset(eloc, new Vector(0,-4,0));
									eloc = Local.lookAt(eloc, e.getLocation());
									e.teleport(eloc);
									player.teleport(Local.offset(eloc,new Vector(-1,0,0)));
									e.setNoDamageTicks(0);
							...
							delay(()->{
								ARSystem.playSound((Entity)player, "c100s2");
								player.teleport(Local.lookAt(e.getLocation().add(0,2,0),e.getLocation()));
								player.setFallDistance(0);
							},85);
							delay(()->{
								ARSystem.playSound((Entity)player, "c100s1");
	...
	public boolean entitydamage(EntityDamageByEntityEvent e, boolean isAttack) {
		if(isAttack) {
			if(e.getDamage() <= 1 && e.getDamage() > 0) {
				...
				Location lc = Local.lookAt(Local.offset(loc, new Vector(2,0,0)),e.getEntity().getLocation());
			...
			if(e.getDamage() <= 1 && ps <= 0) {
				...
				e.setDamage(0);
				e.setCancelled(true);
				player.teleport(Local.lookAt(e.getDamager().getLocation().add(AMath.random(3)-2,0,AMath.random(3)-2),e.getDamager().getLocation()));
