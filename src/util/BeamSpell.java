...
public class BeamSpell extends InstantSpell implements TargetedLocationSpell {
	...
	class Beam {
		...
		public Beam(Player caster, Location from, float power) {
			...
			this.startLoc.setYaw(this.startLoc.getYaw() + (spread/2 - (AMath.random((int) (spread*100)))/100));
			this.startLoc.setPitch(this.startLoc.getPitch() + (spread/2 - (AMath.random((int) (spread*100)))/100));
